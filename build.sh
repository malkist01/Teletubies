...
if [ "$TOOLCHAIN" == gcc ]; then
	...
	then
		...
		git clone --depth=1 https://gitlab.com/LeCmnGend/clang.git -b clang-19 "$HOME"/proton_clang
	fi
...
if [ "$TOOLCHAIN" == clang  ]; then
	...
	make -j$(nproc --all) O=out \
                              ...
                              STRIP=llvm-strip \
                              CC=clang \
                              TRIPLE_COMPILE=aarch64-linux-gnu- \
...
export KBUILD_COMPILER_STRING="$COMP"
...
tg_post_msg "Successful triggered Compiling kernel for mido" "$CHATID"
...
KERVER=$(make kernelversion)
        ...
        if [ -f "$IMG" ]; then
                ...
                curl -sLo zipsigner-3.0.jar https://raw.githubusercontent.com/malkist01/anykernel/master/zipsigner-3.0.jar
